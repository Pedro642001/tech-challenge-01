...
from app.dtos.book import BookDto
...
@router.get("/", description="Lista todos os livros disponíveis", response_model=list[BookDto])
async def get_books(
...
@router.get("/search", description="Busca filtrada por livros", response_model=list[BookDto])
...
@router.get("/{id}", description="Busca livro por ID", response_model=BookDto)
